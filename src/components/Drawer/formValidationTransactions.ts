...
const validationNewTransactionForm = yup.object().shape({
  ...
  dateEntriesTransaction: yup
    ...
    .max(
      yup.ref("dateDueTransaction"),
      "A data de lançamento não pode ser superior ao vencimento."
    )
  ...
  creditorDebtor: yup
    .string()
    .required("A empresa responsável do lançamento é obrigatória!"),
  anotherInformation: yup.string(),
});

export default validationNewTransactionForm;
