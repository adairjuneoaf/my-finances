...
const validationTransactionForm = yup.object().shape({
  ...
  title: yup.string().required("Informe um título para o lançamento.").min(8, "O mínimo de caracteres é 8."),
  description: yup
    .string()
    .required("Informe uma descrição para o lançamento.")
    .min(12, "O mínimo de caracteres é 12."),
  valueTransaction: yup.number().required("Informe o valor do lançamento.").min(0.01, "O valor mínimo é R$ 0.01"),
