...
const DashboardPage: NextPage = () => {
  ...
  useMemo(() => {
    const transactionsInput = data?.filter(
      (transaction) => transaction.type === "1"
    );
    const transactionsOutput = data?.filter(
    ...
    TotalBalance.current =
      Number(TotalInput.current) - Number(TotalOutput.current);
  }, [data]);
  ...
  return (
    <Fragment>
      ...
      <Flex width="100vw" height="auto" flexDirection="column" maxHeight="86vh">
        <HeaderComponent />
        <DrawerComponent />
